...
type Form = { code: string; description: string; sortOrder: string }
const EMPTY: Form = { code: "", description: "", sortOrder: "" }

export function PrefixesTab() {
  ...
  async function load() {
    setLoading(true)
    try { setRows(await api.prefixes.list()) }
  ...
  function openEdit(p: Prefix) {
    setEditing(p)
    setForm({ code: p.code, description: p.description ?? "", sortOrder: String(p.sortOrder) })
    setOpen(true)
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) { await api.prefixes.update(editing.id, data); toast.success("Prefix updated") }
      else { await api.prefixes.create(data); toast.success("Prefix created") }
      setOpen(false); load()
  ...
  async function handleDelete(id: number) {
    ...
    try { await api.prefixes.delete(id); toast.success("Prefix deleted"); load() }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{rows.length} work type prefixes</p>
        <Button size="sm" onClick={openAdd}><Plus className="mr-1.5 h-4 w-4" />Add Prefix</Button>
      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle>{editing ? "Edit Prefix" : "Add Prefix"}</DialogTitle></DialogHeader>
          <div className="grid gap-3 py-1">
            <div className="space-y-1.5">
              <Label>Code * <span className="text-muted-foreground text-xs">(auto-uppercased)</span></Label>
              <Input placeholder="DEV" value={form.code} onChange={(e) => set("code", e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>Description</Label>
              <Input placeholder="Development work" value={form.description} onChange={(e) => set("description", e.target.value)} />
            </div>
            <div className="space-y-1.5">
